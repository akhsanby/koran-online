...
export function getNewsDataFromState(useSelector, router) {
  return useSelector((state) => {
    ...
    if (
      ...
      return state.news.data?.programming;
    if (
      router.pathname === "/covid19" ||
      router.pathname === "/covid19/[detail]"
    )
      return state.news.data?.covid19;
    if (
      router.pathname === "/entertainment" ||
      router.pathname === "/entertainment/[detail]"
    )
      return state.news.data?.entertainment;
    ...
    if (
      ...
      return state.news.data?.technology;
    if (
      (router.pathname === "/search" && router.query.keyword) ||
      router.pathname === "/search/[detail]"
    )
      return state.news.data?.indonesia;
