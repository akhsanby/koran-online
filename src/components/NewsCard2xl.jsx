...
export default function NewsCard2xl({ oneArticle, router }) {
  ...
  const IconBookmarkOrUnBookmark = () => {
    const result = savedNews.some(
      (item) => item.article.title === oneArticle?.title
    );
    if (result)
      return (
        <BookmarkIcon
  ...
  return (
    <div className="h-[480px] xs:h-[600px] relative block group bg-black">
      <Image
        ...
        objectFit="cover"
        src={
          `/api/imageproxy?url=${encodeURIComponent(oneArticle?.urlToImage)}` ||
          null
        }
      ...
      <button
        ...
        onClick={() => handleBookmarkAndUnBookmark(dispatch, oneArticle)}
      >
        <IconBookmarkOrUnBookmark />
      </button>
      <div
        className="absolute left-0 bottom-0 p-5 bg-gradient-to-t from-black via-gray-900/75 cursor-pointer"
        onClick={() => handleRedirectToDetailNews(dispatch, router, oneArticle)}
      >
